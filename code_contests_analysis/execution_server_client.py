...
class ExecutionServerClient:
    ...
    def __enter__(self):
        ...
        try:
            ...
            self.container = self.docker_client.containers.run(
                ...
                # Delete container when we stop it.
                auto_remove=True,
            )
    ...
    def stop_container(self):
        if self.container is not None:
            # auto_remove is True so we don't need to delete it.
            self.container.stop()
    ...
    def _build_new_image(self) -> docker.models.images.Image:
        ...
        try:
            image, _ = self.docker_client.images.build(
                ...
                path=str(self.dockerfile_path.parent.parent),
                tag=IMAGE_NAME,
            )
    ...
    def execute_code(
        ...
        Args:
            ...
            inputs (List[str]): List of input strings for the code.
            timeout (float): Maximum execution time for each input.
            memory_limit_bytes (int): memory limit of the program.

        Returns:
            List[Dict[str, Any]]: A list of dictionaries containing execution results.
        ...
        if not isinstance(results, list):
            raise ValueError(
                f"/execute should return a list, it returned a {type(results.type)}"
            )

        results_parsed = [ExecuteCodeResult(**result) for result in results]
